...
const Home = () => {
  ...
  const { homePageData, loadingHomePage: loadingHome } = useContext(SiteContentContext)
  ...
  const processHomePageBlocks = () => {
    if (!homePageData || !homePageData.blocks || !Array.isArray(homePageData.blocks)) {
    ...
    const heroSectionBlock = homePageData.blocks.find(block => block.blockType === 'hero_section')
    
    // Find shop by category block
    const categoryBlock = homePageData.blocks.find(block => block.blockType === 'shop_by_category')
    
    // Find featured products block
    const featuredProductsBlock = homePageData.blocks.find(block => block.blockType === 'featured_products')
    
    // Find promotional banner blocks
    const promotionalBanners = homePageData.blocks.filter(block => block.blockType === 'promotional_banner')
    
    // Find newsletter signup block
    const newsletterBlock = homePageData.blocks.find(block => block.blockType === 'newsletter_signup')
    
    // Extract specific promotional banners by order or other identifiers
    const summerFestiveBlock = promotionalBanners.find(banner => 
      banner.headline?.includes('Summer') || banner.order === 3)
    
    const workAnywhereBlock = promotionalBanners.find(banner => 
      banner.headline?.includes('Work') || banner.order === 4)
    
    const readySetSummerBlock = promotionalBanners.find(banner => 
      banner.headline?.includes('Dresses') || banner.order === 6)
    
    const playfulFloralsBlock = promotionalBanners.find(banner => 
      banner.headline?.includes('Playful') || banner.order === 7)
    
    const plusSizeCollectionsBlock = promotionalBanners.find(banner => 
      banner.headline?.includes('Inclusive') || banner.order === 8)
      
    const festivalBlock = promotionalBanners.find(banner => 
      banner.headline?.includes('Festival') || banner.order === 5)
  ...
  useEffect(() => {
    ...
    // Set state with processed data
     ...
     if (processedData.heroSection) {
       ...
       if (processedData.heroSection.slides && Array.isArray(processedData.heroSection.slides) && processedData.heroSection.slides.length > 0) {
         ...
         if (processedData.heroSection.mobile_background_image?.url) {
           setMobileHeroImages([processedData.heroSection.mobile_background_image.url]);
         } else {
         ...
         setMobileHeroImages(['https://via.placeholder.com/800x1200?text=No+Image']);
       }
     } else {
    ...
    if (processedData.summerFestiveBannerData && processedData.summerFestiveBannerData.banner_image) {
      setSummerFestiveBanner({
        ...
        alt: processedData.summerFestiveBannerData.banner_image.alt || 'Summer Collection'
      });
    }
    
    if (processedData.workAnywhereBannerData && processedData.workAnywhereBannerData.banner_image) {
      setWorkAnywhereBanner({
        ...
        alt: processedData.workAnywhereBannerData.banner_image.alt || 'Work Collection'
      });
    }
    
    if (processedData.festivalBannerData && processedData.festivalBannerData.banner_image) {
      setFestivalBanner({
        ...
        alt: processedData.festivalBannerData.banner_image.alt || 'Festival Collection'
      });
    }
    
    if (processedData.readySetSummerBannerData && processedData.readySetSummerBannerData.banner_image) {
      setReadySetSummerBanner({
        ...
        alt: processedData.readySetSummerBannerData.banner_image.alt || 'Summer Dresses Collection'
      });
    }
    
    if (processedData.playfulFloralsBannerData && processedData.playfulFloralsBannerData.banner_image) {
      setPlayfulFloralsBanner({
        ...
        alt: processedData.playfulFloralsBannerData.banner_image.alt || 'Floral Collection'
      });
    }
    
    if (processedData.plusSizeCollectionsBannerData && processedData.plusSizeCollectionsBannerData.banner_image) {
      setPlusSizeCollectionsBanner({
        ...processedData.plusSizeCollectionsBannerData,
        desktopImage: processedData.plusSizeCollectionsBannerData.banner_image.url,
        mobileImage: processedData.plusSizeCollectionsBannerData.banner_image.url,
        alt: processedData.plusSizeCollectionsBannerData.banner_image.alt || 'Plus Size Collection'
      });
    }
  ...
  useEffect(() => {
    const fetchLatestProducts = async () => {
      try {
        ...
        const response = await api.get('/products?sort=createdAt&order=desc&limit=6')
        if (response.data.success) {
          ...
          const products = response.data.data.products.map(product => ({
            ...
            image: product.images && product.images.length > 0 ? product.images[0] : 'https://via.placeholder.com/400x500',
  ...
  useEffect(() => {
    const fetchMostPurchasedProducts = async () => {
      try {
        ...
        const topProducts = allProducts.filter(product => topProductIds.includes(product._id))
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (heroSection?.slides && heroSection.slides.length > 0) {
        setCurrentSlide((prev) => (prev === heroSection.slides.length - 1 ? 0 : prev + 1))
  ...
  }, [summerFestiveBanner, workAnywhereBanner, festivalBanner, readySetSummerBanner, playfulFloralsBanner, plusSizeCollectionsBanner]);
  
  // Handle manual slide navigation
  const goToSlide = (index) => {
    if (index >= 0 && ((heroSection?.slides && index < heroSection.slides.length) || (heroImages.length > 0 && index < heroImages.length))) {
  ...
  const prevSlide = () => {
    if (heroSection?.slides && heroSection.slides.length > 0) {
      setCurrentSlide((prev) => (prev === 0 ? heroSection.slides.length - 1 : prev - 1))
    } else if (heroImages.length > 0) {
    ...
    <div>
        ...
        {heroSection?.slides && heroSection.slides.length > 0 ? (
              ...
              >
                <picture>
                  ...
                  <img 
                    ...
                    alt={heroSection.slides[currentSlide]?.desktop_image?.alt || 
                ...
                <div className={`absolute inset-0 ${heroSection.slides[currentSlide]?.overlay_style === 'light' ? 'bg-white/30' : 'bg-black/30'} flex items-center justify-center`}>
                  <div className="text-center text-white max-w-4xl px-4">
                    <h1 className="text-3xl md:text-5xl font-bold mb-2 md:mb-4">
                      {heroSection.slides[currentSlide]?.headline || "Summer Collection 2023"}
                    </h1>
                    <p className="text-base md:text-xl mb-4 md:mb-8">
                      {heroSection.slides[currentSlide]?.subheadline || "Discover the latest trends for the season"}
                    </p>
                    {heroSection.slides[currentSlide]?.cta_text && (
                      <Button
                        text={heroSection.slides[currentSlide].cta_text || "Shop Now"}
                        link={heroSection.slides[currentSlide].cta_link || "/shop"}
          ...
          <div className="flex items-center justify-center h-full bg-gray-200">
            <div className="text-center text-gray-600 max-w-4xl px-4">
              ...
              <p className="text-base md:text-xl mb-4 md:mb-8">
                {heroSection?.subheadline || "Discover the latest trends for the season"}
              </p>
              {heroSection?.cta && (
                <Button
                  text={heroSection.cta.label || "Shop Now"}
                  link={heroSection.cta.url || "/shop"}
        ...
        <div className="container-fluid mx-auto px-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-0">
            {categories.map((category, index) => (
              <div key={index} className="relative overflow-hidden h-[350px] md:h-[500px] lg:h-[700px] group">
                <img
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent flex flex-col justify-end p-4 md:p-6">
                  <h3 className="text-white text-xl md:text-2xl font-bold mb-2 md:mb-4 uppercase tracking-wider text-center">{category.label}</h3>
                  <a href={category.link} className="text-white text-center hover:underline text-sm md:text-base">Shop Collection</a>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-10 md:py-14 lg:py-16 bg-gray-50">
        <div className="container-custom">
          <h2 className="text-2xl md:text-3xl font-heading font-semibold mb-6 md:mb-8 lg:mb-10 text-center">Latest Arrivals</h2>
          {loadingLatestProducts ? (
            ...
            <motion.div 
              ...
              animate="visible"
            >
              {latestProducts && latestProducts.length > 0 ? latestProducts.map((product, index) => {
                    ...
                    <div className="p-3 xs:p-4 sm:p-5">
                      ...
                      <div className="flex justify-between items-start mb-2.5">
                        <Link to={`/product/${product.slug || product.id}`} className="block flex-1">
                          <h3 className="text-sm xs:text-base md:text-lg font-medium text-gray-800 hover:text-java-500 transition-colors line-clamp-2">{product.title}</h3>
                        </Link>
                        ...
                        {product.rating && (
                          <div className="flex items-center bg-gradient-to-r from-java-100 to-java-200 px-1.5 xs:px-2 py-0.5 rounded-full ml-1 xs:ml-2">
                            ...
                            <span className="text-[10px] xs:text-xs font-medium text-java-800">{product.rating}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-baseline gap-1 xs:gap-1.5">
                          <p className="text-sm xs:text-base md:text-lg font-bold text-red-500">₹{(product.price && product.price !== null) ? product.price.toFixed(2) : '0.00'}</p>
                          {product.originalPrice && product.originalPrice > product.price && (
                            <p className="text-[9px] xs:text-[10px] sm:text-xs text-gray-400 line-through">₹{(product.originalPrice && product.originalPrice !== null) ? product.originalPrice.toFixed(2) : '0.00'}</p>
                          )}
                        </div>
                      </div>
                        ...
                        <div className="mt-1">
                          ...
                          <div className="flex flex-wrap gap-1 xs:gap-1.5">
                            {product.sizes.map((size, sizeIndex) => (
                              <span key={sizeIndex} className="inline-block px-1.5 xs:px-2 py-0.5 text-[10px] xs:text-xs bg-gray-50 border border-gray-200 rounded-full text-gray-700 hover:bg-java-50 hover:border-java-200 transition-all cursor-pointer">{size}</span>
                            ))}
                          </div>
                        </div>
                      )}
        ...
        <div className="container mx-auto px-4">
          {bannerDebug.readySetSummer && (
            <div className="bg-yellow-100 p-4 mb-4 rounded">
              <p className="font-mono text-sm">Debug: {JSON.stringify(readySetSummerBanner, null, 2)}</p>
            </div>
          )}
          ...
          {readySetSummerBanner && readySetSummerBanner.banner_image ? (
            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="md:w-1/2 space-y-4">
                ...
                >
                  {readySetSummerBanner.subheadline || "Discover our new collection of summer dresses that will keep you cool and stylish all season long."}
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                  <Link to={readySetSummerBanner.cta_link || "/products/category/dresses"} className="inline-block bg-primary text-white px-8 py-3 rounded-md font-medium hover:bg-primary-dark transition duration-300">
              ...
              <div className="md:w-1/2">
                ...
                >
                  <picture>
                    <source media="(max-width: 767px)" srcSet={readySetSummerBanner.banner_image.url} />
                    <img 
                      src={readySetSummerBanner.banner_image.url} 
                      alt={readySetSummerBanner.banner_image.alt || "Ready Set Summer Collection"} 
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="md:w-1/2 space-y-4">
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.5, delay: 0.2 }}
                >
                  <Link to="/products/category/dresses" className="inline-block bg-primary text-white px-8 py-3 rounded-md font-medium hover:bg-primary-dark transition duration-300">
                    Shop Dresses
                  </Link>
                </motion.div>
              </div>
      ...
      <section className="playful-florals-banner py-16">
        <div className="container mx-auto px-4">
          {bannerDebug.playfulFlorals && (
            <div className="bg-yellow-100 p-4 mb-4 rounded">
              <p className="font-mono text-sm">Debug: {JSON.stringify(playfulFloralsBanner, null, 2)}</p>
            </div>
          )}
            ...
            <div className="flex flex-col md:flex-row-reverse items-center justify-between gap-8">
              <div className="md:w-1/2 space-y-4">
                ...
                <motion.p
                  ...
                  className="text-lg text-gray-600"
                >
                  {playfulFloralsBanner.subheadline || "Embrace the season with our vibrant floral prints that add a touch of nature to your wardrobe."}
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5, delay: 0.2 }}
                >
                  <Link to={playfulFloralsBanner.cta_link || "/products/category/floral"} className="inline-block bg-primary text-white px-8 py-3 rounded-md font-medium hover:bg-primary-dark transition duration-300">
              ...
              <div className="md:w-1/2">
                ...
                >
                  <picture>
                    <source media="(max-width: 767px)" srcSet={playfulFloralsBanner.banner_image.url} />
                    <img 
                      src={playfulFloralsBanner.banner_image.url} 
                      alt={playfulFloralsBanner.banner_image.alt || "Floral Collection"} 
                      className="w-full h-auto object-cover"
                    />
                  </picture>
                </motion.div>
            ...
            <div className="flex flex-col md:flex-row-reverse items-center justify-between gap-8">
              <div className="md:w-1/2 space-y-4">
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.5, delay: 0.2 }}
                >
                  <Link to="/products/category/floral" className="inline-block bg-primary text-white px-8 py-3 rounded-md font-medium hover:bg-primary-dark transition duration-300">
      ...
      <section className="plus-size-collections-banner py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          {bannerDebug.plusSizeCollections && (
            <div className="bg-yellow-100 p-4 mb-4 rounded">
              <p className="font-mono text-sm">Debug: {JSON.stringify(plusSizeCollectionsBanner, null, 2)}</p>
            </div>
          )}
          
          {plusSizeCollectionsBanner && plusSizeCollectionsBanner.banner_image ? (
            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="md:w-1/2 space-y-4">
                <motion.h2 
                  initial={{ opacity: 0, y: 20 }}
                  ...
                  className="text-4xl md:text-5xl font-bold text-gray-800"
                >
                  {plusSizeCollectionsBanner.headline || "Inclusive Fashion For All"}
                </motion.h2>
                <motion.p
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  ...
                  className="text-lg text-gray-600"
                >
                  {plusSizeCollectionsBanner.subheadline || "Our plus size collection celebrates every body with stylish, comfortable pieces designed to make you look and feel amazing."}
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5, delay: 0.2 }}
                >
                  <Link to={plusSizeCollectionsBanner.cta_link || "/products/category/plus-size"} className="inline-block bg-primary text-white px-8 py-3 rounded-md font-medium hover:bg-primary-dark transition duration-300">
              ...
              <div className="md:w-1/2">
                ...
                >
                  <picture>
                    <source media="(max-width: 767px)" srcSet={plusSizeCollectionsBanner.banner_image.url} />
                    <img 
                      src={plusSizeCollectionsBanner.banner_image.url} 
                      alt={plusSizeCollectionsBanner.banner_image.alt || "Plus Size Collection"} 
                      className="w-full h-auto object-cover"
                    />
                  </picture>
                </motion.div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="md:w-1/2 space-y-4">
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.5, delay: 0.2 }}
                >
                  <Link to="/products/category/plus-size" className="inline-block bg-primary text-white px-8 py-3 rounded-md font-medium hover:bg-primary-dark transition duration-300">
                    Shop Plus Size
                  </Link>
                </motion.div>
              </div>
      ...
      <section className="py-10 md:py-14 lg:py-16 bg-gray-50">
        <div className="container-custom">
          <h2 className="text-2xl md:text-3xl font-heading font-semibold mb-6 md:mb-8 lg:mb-10 text-center">Most Purchased Products</h2>
          ...
          {loadingMostPurchasedProducts ? (
            ...
            <motion.div 
              ...
              animate="visible"
            >
              {mostPurchasedProducts && mostPurchasedProducts.length > 0 ? mostPurchasedProducts.map((product) => {
                // Calculate discount percentage if applicable
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-3 xs:p-4 sm:p-5">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-baseline gap-1 xs:gap-1.5">
                          <p className="text-sm xs:text-base md:text-lg font-bold text-red-500">₹{product.price ? product.price.toFixed(2) : '0.00'}</p>
                          {product.originalPrice && (
                            <p className="text-[9px] xs:text-[10px] sm:text-xs text-gray-400 line-through">₹{product.originalPrice ? product.originalPrice.toFixed(2) : '0.00'}</p>
                          )}
                        </div>
                        {discountPercentage > 0 && (
      ...
      {/* Newsletter Section */}
      <section className={`py-16 ${homePageData?.newsletter_signup?.use_teal_background ? 'bg-teal' : 'bg-gray-100'} ${homePageData?.newsletter_signup?.use_white_text ? 'text-white' : 'text-gray-800'}`}>
        <div className="container-custom text-center">
          <h2 className="text-3xl font-heading font-semibold mb-4">{homePageData?.newsletter_signup?.headline || newsletter.headline || "Newsletter"}</h2>
          <p className="text-lg mb-8 max-w-2xl mx-auto">{homePageData?.newsletter_signup?.subtext || newsletter.description || "Subscribe to our newsletter for updates."}</p>
          
          {subscriptionSuccess ? (
            <div className={`${homePageData?.newsletter_signup?.use_teal_background ? 'bg-white text-teal' : 'bg-teal text-white'} p-4 rounded-md max-w-md mx-auto`}>
              <p className="text-lg font-medium">{homePageData?.newsletter_signup?.success_message || "Thank you for subscribing!"}</p>
            </div>
          ) : (
            <form className="max-w-md mx-auto flex" onSubmit={handleNewsletterSubmit}>
              <input 
                type="email" 
                placeholder={homePageData?.newsletter_signup?.placeholder_text || "Your email address"} 
              ...
              <Button type="submit" className="rounded-l-none">
                {homePageData?.newsletter_signup?.button_text || newsletter.button_text || "Subscribe"}
              </Button>
            </form>
          )}
        </div>
